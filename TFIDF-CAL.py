...
if (os.path.isdir(TFIDF_Features_Path) and os.path.isdir(Text_Features_Path)):
    ...
    if len(text_files)>2: 
        ...
        Tfidf_vectorizer = TfidfVectorizer(input='filename', lowercase=True, analyzer = 'word', decode_error='ignore', stop_words=stpwrd,max_features=100, token_pattern=regex1, min_df=5)
        ...
        pickle.dump(Tfidf_vectorizer.vocabulary_,open(TFIDF_Model_Path,"wb"))
    else:
